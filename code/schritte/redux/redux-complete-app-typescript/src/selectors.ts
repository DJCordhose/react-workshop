...
export const filterGreetings =
	(greetings: Greetings, filter: GreetingFilter): Greetings =>
...
export const aggregateGreetings = (greetings: Greetings): GreetingChartData => {
	...
	return Object.keys(dataAsObjectMap).
		map(key => ({ label: key, value: dataAsObjectMap[key] }));
};
